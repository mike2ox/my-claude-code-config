...
| 단계 | 커맨드 | 설명 |
|------|--------|------|
| 1. 기획 | `/my-plan [기능명]` | 기능 아이디어와 설계 방향 정리 (plan 모드) |
| 2. 작업 분해 | `/my-split` | plan → 주니어 친화적 step별 작업 분해 |
| 3. 커밋 | `/my-commit` | step 완료마다 attribution 없이 커밋 |
| 4. 완료 검사 | `/my-check` | 타입 체크 후 최적화 기회 검토 |
| 5. PR 제목 | `/my-pr` | 브랜치 변경 분석 후 PR 제목 추천 |
| 6. 리뷰 반영 | `/my-review [피드백]` | 코드 리뷰 피드백 분류 및 적용 |
| 7. 회고 | `/my-retro` | 작업 회고 문서 생성 (브랜치 삭제 전) |
```
